...
| `--force` | Content conflict ⇒ playbook wins (re-link dest). Bidirectional paths otherwise prompt playbook vs target. |
...
- Same inode: no-op. Same bytes, different inode: re-link. Different bytes: `.dev-notes/` and project `dev-guide.md` prompt playbook vs target; `.cursor/` and `--machine` prompt playbook-only; `--force` ⇒ playbook.
